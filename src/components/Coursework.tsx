...
const Coursework: React.FC = () => {
  ...
  return (
    <section id="coursework" className="max-w-5xl mx-auto px-6 py-12">
      ...
      {/* Tab Navigation */}
      <div className="flex flex-row max-md:flex-col mb-8 border-b border-gray-300 dark:border-gray-800">
        {tabs.map((tab) => (
          <button
            key={tab.id}
          ...
          >
            {courseData[activeTab].map((course, index) => (
              ...
              >
                ...
                {/* Course Title */}
                <span className="text-gray-700 dark:text-gray-400 text-lg leading-relaxed">
                  {course.title}
                </span>
              </motion.div>
            ))}
